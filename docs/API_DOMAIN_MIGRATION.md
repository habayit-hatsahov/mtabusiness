...
אוהד מאושר (שחר קאשי, 0••-•••••••) לא הצליח להיכנס בחמישה ניסיונות ב-1.9. הוורקר עצמו
...
✅ **אושר סופית באותו יום:** שחר **התחבר ל-Wi-Fi ונכנס מיד**. אותו מכשיר, אותו קוד, אותם
פרטים, 40 דקות אחרי חמישה כשלים רצופים — **רק הרשת השתנתה.** זה המבחן שמבודד את המשתנה,
ולכן ההשערה הפכה לוודאות: הרשת הסלולרית שלו מסננת את הכתובת, ה-Wi-Fi לא.

הסייג שנרשם כאן קודם — "אולי זה נתיב-רשת שבור אל Cloudflare, ואז הכתובת החדשה לא תעזור" —
**נשלל**: נתיב שבור אל Cloudflare לא היה מתוקן ע"י מעבר ל-Wi-Fi על אותו מכשיר. סינון
סלולרי פועל **לפי שם-הדומיין**, ושם שאינו על רשימות-הסינון עוקף אותו.

⚠️ **וזו לא תקלה של אדם אחד:** רוב האוהדים גולשים מהסלולר. שחר הוא הראשון שדיווח, לא
בהכרח הראשון שנתקע — `loginFail` נאסף רק מ-30.8.

שלב 6 בכל זאת משאיר את `workers.dev` כנפילה-לאחור: לא בגלל ספק באבחנה, אלא כדי שתקלה
בכתובת החדשה לא תנעל את **כל** האוהדים בחוץ.
...
### שלב 2ב — ⚠️ בדיקה מקדימה: מה Cloudflare **היה** עונה, לפני שהוא אוטוריטטיבי

**זה השלב שהופך את המהלך מ"מסוכן" ל"מאומת", והוא עולה שתי דקות.**

שרתי-השמות של Cloudflare עונים על הדומיין **מהרגע שהוספת אותו**, גם לפני שהרשם מפנה
אליהם. אפשר לשאול אותם ישירות ולראות בדיוק מה יקרה אחרי ההחלפה — בזמן שהאתר עדיין רץ
על sitesdepot ואי אפשר לשבור כלום.

לשים את שם שרת-השמות ש-Cloudflare הקצה במקום `<NS>`:

```bash
nslookup -type=MX yellowzone.co.il <NS>
```
```bash
nslookup -type=TXT yellowzone.co.il <NS>
```
```bash
nslookup -type=A mail.yellowzone.co.il <NS>
```
```bash
nslookup -type=A yellowzone.co.il <NS>
```
```bash
nslookup -type=CNAME brevo1._domainkey.yellowzone.co.il <NS>
```

כל תשובה חייבת להיות **זהה** לטבלה בסעיף 2. תשובה ריקה = רשומה שלא הוזנה, ואחרי
ההחלפה היא פשוט תיעלם.

🔑 **רשומה חסרה שמתגלה כאן היא תיקון של דקה. אותה רשומה חסרה שמתגלה אחרי שלב 3 היא
מייל מושבת עד שמישהו שם לב.**
...
⏱️ **רק השלב הזה צריך שעה שקטה** (ערב), והוא היחיד שנוגע באתר החי. שלבים 1–2ב הם עבודת
תצורה שאינה פעילה עדיין — **עדיף לעשות אותם לאור יום וברוגע**, כי שלב 2 הוא הרגיש
מכולם (11 רשומות, 5 מהן מייל) ואסור לעשות אותו עייף.

⚠️ **בזמן ההעברה עצמה אין נפילה:** כל עוד הרשומות הועתקו נכון, שני צדדי ה-DNS מגישים
בדיוק את אותו מידע. הסיכון היחיד הוא רשומה שנשכחה — וזה בדיוק מה ששלב 2ב תופס.
...
לחזור לשחר קאשי (`•••@•••`, 0••-•••••••) ולבקש שינסה שוב — הוא מקרה-הבוחן
