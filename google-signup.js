...
(function () {
  ...
  // ── §412 — הבטחה קטנה הוחלפה בהבטחה האמיתית ──────────────────────────────────────────
  // §411 הבטיח *"השם והמייל מתמלאים לבד"*. המשתמש הצביע על החולשה: **זה שני שדות מתוך
  // שמונה** — הבטחה מדידה, וקטנה מכדי לשנות החלטה. התועלת האמיתית היא היחידה שכל הערב
  // הזה מדד: **הקוד הוא מה שאנשים שוכחים** (§399ב — חבר מאושר ששכח את הקוד לחץ על גוגל,
  // קיבל הודעה, ונרשם מחדש; §398 — 8 מתוך 8 שלחצו על גוגל עשו זאת בלי להיות מחוברים).
  //
  // ⚠️ **ו"הרשמה" בפתח הוא תיקון נפרד וחשוב לא פחות:** גוגל מחליפה את הכיתוב על הכפתור
  // שביקשנו (`signup_with`) בכפתור מותאם-אישית — *"כניסה בשם רמי"* — לכל מי שכבר מחובר
  // לחשבון גוגל בדפדפן, כלומר רוב הגולשים בנייד. התוצאה: בטופס **הרשמה** מופיע כפתור
