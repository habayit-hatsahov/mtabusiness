...
(function () {
  ...
  // ── ✅ §445 — הודלק (20.9.2026) ───────────────────────────────────────────────────────
  // ה-Services ID נוצר בפורטל של אפל אחרי שהחשבון אושר, **ואומת חי מקצה לקצה** לפני
  // ההדלקה: טוקן אמיתי מגיליון אפל → `/apple-login` → `apple_not_linked` עם
  // `aud: il.co.yellowzone.web`. ר' §445 ו-`apple-live-test.html`.
  // 🔑 **זהו המתג היחיד**: ריק = הבלוק אינו מוצג באף דף. מלא = מוצג בכל דף שהמודול
  // משולב בו, בלי לגעת באף דף.
  // ⚠️ אינו סוד — משובץ גלוי בקוד הלקוח, כמו ה-Client ID של גוגל.
  //
  // 🔴 **§446ו — הערך הזה חי בארבעה מקומות, והם נעולים זה לזה בבדיקה:**
  //   `apple-signup.js` (כאן) · `welcome.html` (`HB_APPLE_SERVICES_ID`, מסך הכניסה) ·
  //   `apple-live-test.html` (דף האבחון) · `worker/wrangler.toml` (`APPLE_CLIENT_IDS`).
  // **פער בין כל שניים מהם מפיל כל כניסה עם אפל על שגיאת-audience** — וההודעה שהמשתמש
  // רואה זהה לזו של טוקן פגום, כלומר אין שום רמז לסיבה.
  // 🔑 **ולכן: `node scratch_test_apple_client_id.js` אחרי כל שינוי כאן.** הוא נועל את
  // ארבעתם, וגם סורק את הריפו כדי לתפוס עותק חמישי שייוולד.
  // ⚠️ **ובמכוון אין כאן מודול-קבועים משותף:** הערך בוורקר הוא **רשימה** (הוא נושא גם את
  // ה-Bundle ID של iOS) והוא נפרס בנפרד ב-`wrangler deploy`, ו-`welcome.html` אינו טוען
  // את הקובץ הזה בכוונה (הוא מודול הרשמה). מודול משותף היה סוגר את הצמד הבטוח ומשאיר
  // פתוח בדיוק את הצד שנפרס לחוד — זה שבאמת נשכח.
  ...
  // ══ §446ב — 🔴 בתוך האפליקציה הכפתור **לא מוצג** ═══════════════════════════════════════
  //
  // **הפער שזה סוגר, והוא היה חי:** `google-signup.js` קיבל את השער הזה ב-§435, אחרי
  // ש-§434 מדד על מכשיר מדוע כפתור גוגל מוביל לדף לבן. `apple-signup.js` נכתב ב-§424 —
  // **לפני** §434 — והודלק ב-§445 בלי שאיש חזר לשאול אם אותו תנאי חל גם עליו. התוצאה:
  // בשני טופסי ההרשמה, בתוך אפליקציית האנדרואיד, כפתור גוגל כבר הוחלף לנייטיב בדיוק
  // בגלל התקלה — וכפתור אפל **המשיך להיות מצויר** לצידו.
  //
  // 🔑 **המנגנון זהה לחלוטין ואינו תלוי בספק:** `AppleID.auth.signIn()` עם `usePopup:true`
  // פותח `window.open` אל `appleid.apple.com`, ו-`Bridge.launchIntent` של Capacitor שולח
  // **כל host שאינו yellowzone.co.il** לדפדפן החיצוני. שם אין `window.opener`, ולכן
  // הטוקן לא חוזר לאפליקציה — דף לבן, והנרשם נשאר בלי כלום.
  //
  // ⚠️ **וזו גזירה מהמנגנון שנמדד ב-§434, ולא מדידה של אפל עצמה על מכשיר** —
  // ר' [[feedback_absence_of_evidence]]. אבל המחיר של הסתרה הוא אפס (אין ממילא מסלול
  // אפל באפליקציה), והמחיר של טעות לכיוון השני הוא בדיוק התקלה שהבודקים כבר דיווחו עליה.
  //
  // ⚠️ **הנפילה-לאחור ל-UA אינה כפילות מיותרת**, בדיוק כמו ההערה ב-`google-signup.js`:
  // אם `native-google.js` חסר (HTML ישן מהמטמון מול JS חדש) — בלעדיה היינו חוזרים לצייר.
  // 🔗 **הפתרון האמיתי הוא מסלול נייטיב ולא תיקון של המסלול הזה** — תוסף Capacitor שמחזיר
  // `identityToken` שהוורקר מאמת באותו `apple.js` (ה-`aud` הוא ה-Bundle ID, ולכן
  // `APPLE_CLIENT_IDS` הוא רשימה מלכתחילה). §426 — חסום עד שיהיה `app/ios`.
  function isInApp() {
    var ng = window.YZNativeGoogle;
    if (ng && typeof ng.mode === 'function') return ng.mode() !== 'web';
  ...
  function render(tries) {
    ...
    // §446ב — ⚠️ **לפני `loadSdk()`, וזה לא סדר שרירותי:** ההטענה עולה 42KB לשרת של אפל,
    // ואין שום סיבה לשלם עליהם במכשיר שבו הכפתור לא יוצג ממילא. אותו נימוק של §430.
