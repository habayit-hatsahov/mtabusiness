...
(async () => {
  ...
  // ══ §449ב — המשפט על רשומת הביקורת ═══════════════════════════════════════════════════
  //
  // 🔑 **למה הוא שם:** המימוש של §449 **אינו מוחק** את מסמך היומן אלא מרוקן ממנו את עותק
  // הרשומה ומשאיר קליפת ביקורת. המסמך אמר "העותק נמחק" בלבד — נכון עובדתית אך חלקי,
  // ובדיוק בסעיף שבודק החנות קורא.
  //
  // ⚠️ **והוא חייב להישאר טקסט רגיל.** מנוע הרינדור הופך **בשקט** כל שורה שיש בה
  // נקודתיים ב-40 התווים הראשונים ללייבל מודגש (ר' הבדיקה ההפוכה על סעיף 5). נקודתיים
  // שתיכנס לשורה הזאת בעריכה עתידית תשנה את צורתה בלי שאיש ישים לב.
  // 🐛 **והניסיון הראשון כאן היה הגנה שמתקיימת תמיד.** הבדיקה חיפשה את האלמנט
  // ה"פנימי ביותר שמכיל את המשפט" ואז דרשה שלא יהיה בו `<strong>` — אבל כשהשורה **כן**
  // הופכת ללייבל, ה-`<strong>` עצמו מכיל את תחילת המשפט, ולכן הוא זה שנבחר, ובתוכו
  // אין `<strong>` נוסף. **הבדיקה עברה בירוק בדיוק במקרה שבשבילו נכתבה.**
  // 🔑 **מה שמחליף אותה:** דרישה שהמשפט יופיע **מילה-במילה** בטקסט המרונדר. הכנסת
  // נקודתיים מוסיפה `': '` לפלט, ולכן ההשוואה המדויקת נשברת מיד.
  // ר' [[feedback_guard_that_always_holds]].
  const AUDIT_LINE = 'לאחר מחיקת העותק נשמרת רשומת ביקורת המתעדת את עצם המחיקה';
  check('המשפט על רשומת הביקורת קיים במקור', privacy.includes(AUDIT_LINE));
  const rendered = container.textContent.replace(/\s+/g, ' ');
  check('המשפט מרונדר מילה-במילה (כלומר לא הפך ללייבל מודגש)',
        rendered.includes(AUDIT_LINE),
        rendered.slice(Math.max(0, rendered.indexOf('לאחר מחיקת העותק')), rendered.indexOf('לאחר מחיקת העותק') + 90));
  const auditBold = [...container.querySelectorAll('strong')]
    .map(s => s.textContent.replace(/:\s*$/, '').trim())
    .filter(t => t.length > 3 && AUDIT_LINE.startsWith(t));
  check('אין לייבל מודגש שנחתך מתוך המשפט', auditBold.length === 0, auditBold.join(' | '));
  const auditEl = [...container.querySelectorAll('*')]
    .filter(el => el.textContent.includes(AUDIT_LINE)).pop();
  check('המשפט אינו נבלע לתוך תיבת-ההדגשה שמעליו',
        !!auditEl && !auditEl.closest('.terms-callout'));
  check('המשפט אומר מה לא נשמר, ולא רק מה כן',
        privacy.includes('בלי שם, בלי פרטי קשר ובלי תוכן הרשומה'));
